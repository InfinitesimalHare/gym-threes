...
class ThreesEnv(gym.Env):
  ...
  def step(self, action):
    ...
    if done:
      ...
      +a when merging a and a
        """
      # self.history.append([self.board_h, self.nextTile, self.score, action, reward])
  ...
  def reset(self):
    ...
    # self.history = []
    # self.history.append([self.board_h, self.nextTile, self.score, None, 0])
  ...
  def render(self, mode='uint64', close=False):
    ...
    if mode == 'uint64':
      ...
      figObj = RecordThrees([[self.board_h, self.nextTile, self.score, None, None]])
...
class Game_tkinter:
    def __init__(self,gamepanel: BoardGUI, boardBinary: ThreesEnv ):
        ...
        self.boardBinary = boardBinary # seed 16
    ...
    def link_keys(self,event):
        ...
        if self.end:
          ...
          fileno = len(list(output_dir.glob('*')))
          with open(output_dir/"humangame{}".format(fileno+1), 'wb') as fp:
